  ...
  /*
    ...
    As exceções podem ser verificadas (checked) ou não verificadas (unchecked)
      - Verificadas: São verificadas pelo compilador e devem ser tratadas ou declaradas explicitamente.
        Ex: IOException
      - Não verificadas: Ocorrências em tempo de execução, não são obrigatórias de tratar.
        Ex: NullPointerException;
    O tramento de erros permitem capturar e lidar com erros, mantendo a estabilidade do programa e prevendo encerramento abruptos
  */

  //Bloco try-catch
  /*
    O bloco try-catch permite cpaturar e tratar exceções que ocorrem durante a execução do programa
    Para gerar uma exceção é colocado dentro do bloco try
    O bloco catch captura a exceção e executa o código de tratamento;
    A importancia de usar o try-catch é para que o programa não encerre inesperadamente em caso de erro
  */
  
  //Bloco Finally
  /*
    O bloco Finally é usado em conjunto com try-catch para garantir que o código seja executado , independentemente de uma exceção ter sido lançada ou não
  ...
  */
  ...
  /*
    Exceções Verificadas:
      ...
      - O compilador exige que vocẽ trate ou declare essas exceções com try-catch ou throws
